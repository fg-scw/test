...
class ScalewayInstanceAPI:
    ...
    def create_image(
        ...
        Args:
            extra_snapshots: List of snapshot IDs for additional volumes (data disks).
                             These become extra_volumes in the Scaleway image.
        """
        ...
        # Add extra volumes (data disks)
        if extra_snapshots:
            extra_volumes = {}
            for i, snap_id in enumerate(extra_snapshots):
                extra_volumes[str(i + 1)] = {
                    "id": snap_id,
                    "volume_type": "sbs_snapshot",
                }
            payload["extra_volumes"] = extra_volumes

        logger.info(f"Creating image '{name}' from snapshot {root_volume_snapshot_id}"
                     + (f" + {len(extra_snapshots)} extra volume(s)" if extra_snapshots else ""))
        result = self._request("POST", url, json=payload)
